...
![并行和并发](assets/javathread-1.png)并行和并发

就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。

![并行并发和食堂打饭](assets/javathread-2.png)并行并发和食堂打饭
...
![程序进程线程关系](assets/javathread-3.png)
...
![线程创建三种方式](assets/javathread-4.png)
...
![start方法](assets/javathread-5.png)
...
![线程常用调度方法](assets/javathread-6.png)
...
![Java线程状态变化](assets/javathread-7.png)
...
![线程切换-2020-12-16-2107](assets/javathread-8.png)

<div align="center">线程切换-2020-12-16-2107</div>

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![上下文切换时机](assets/javathread-9.png)
...
![线程间通信方式](assets/javathread-10.png)
...
![ThreadLocal线程副本](assets/javathread-11.png)
...
![ThreadLoca存放用户上下文](assets/javathread-12.png)
...
![ThreadLoca结构图](assets/javathread-13.png)
...
![ThreadLocal内存分配](assets/javathread-14.png)
...
![ThreadLocalMap结构示意图](assets/javathread-15.png)
...
![ThreadLocalMap解决冲突](assets/javathread-16.png)
...
![ThreadLocalMap扩容](assets/javathread-17.png)

<div align="center">ThreadLocalMap扩容</div>

具体代码：

![ThreadLocalMap resize](assets/javathread-18.png)
...
![多级指令重排](assets/javathread-21.png)
...
![双重校验单例模式异常情形](assets/javathread-22.png)
...
![happens-before六大规则](assets/javathread-23.png)
...
![img](assets/javathread-24.png)

A和C之间存在数据依赖关系，同时B和C之间也存在数据依赖关系。因此在最终执行的指令序列中，C不能被重排序到A和B的前面（C排到A和B的前面，程序的结果将会被改变）。但A和B之间没有数据依赖关系，编译器和处理器可以重排序A和B之间的执行顺序。

所以最终，程序可能会有两种执行顺序：

![两种执行结果](assets/javathread-25.png)
...
![volatile内存可见性](assets/javathread-26.png)
...
![volatile重排序规则表](assets/javathread-27.png)
...
![volatile写插入内存屏障后生成的指令序列示意图](assets/javathread-28.png)

<div align="center">volatile写插入内存屏障后生成的指令序列示意图</div>

![volatile写插入内存屏障后生成的指令序列示意图](assets/javathread-29.png)
...
![monitorenter和monitorexit](assets/javathread-30.png)
...
![synchronized修饰同步方法](assets/javathread-31.png)
...
![就诊-图片来源参考[18]](assets/javathread-32.png)
...
![Java Montior机制](assets/javathread-33.png)
...
![Mark Word变化](assets/javathread-34.png)
...
![锁升级方向](assets/javathread-35.png)
...
![锁升级简略过程](assets/javathread-36.png)锁升级简略过程

完整的升级过程：

![synchronized 锁升级过程-来源参考[14]](assets/javathread-37.png)
...
![synchronized和ReentrantLock的区别](assets/javathread-38.png)
...
![ReentrantLock 非公平锁加锁流程简图](assets/javathread-42.png)
...
![公平锁tryAcquire](assets/javathread-43.png)
...
![CAS三大问题](assets/javathread-44.png)
...
![Java保证原子性方法](assets/javathread-45.png)
...
![原子操作类](assets/javathread-46.png)
...
![死锁示意图](assets/javathread-47.png)

<div align="center">死锁示意图</div>

那么为什么会产生死锁呢？ 死锁的产生必须具备以下四个条件：

![死锁产生必备四条件](assets/javathread-48.png)
...
![线程死锁检测](assets/javathread-49.png)
...
![王者荣耀等待玩家确认-来源参考[18]](assets/javathread-50.jpeg)
...
![王者荣耀-来源参考[18]](assets/javathread-51.jpeg)
...
![想结衣结衣不依,迷爱理爱理不理。](assets/javathread-52.png)
...
![小约翰可汗选妃模拟代码](assets/javathread-53.png)

<div align="center">小约翰可汗选妃模拟代码</div>

运行结果：

![运行结果](assets/javathread-54.png)运行结果
...
![CyclicBarrier工作流程](assets/javathread-55.png)
...
![停车场空闲车位提示-图片来源网络](assets/javathread-56.jpeg)
...
![Semaphore许可获取-来源参考[18]](assets/javathread-57.jpeg)
...
![英雄交换猎物-来源参考[18]](assets/javathread-58.jpeg)
...
![管理线程的池子](assets/javathread-59.png)
...
![业务示例](assets/javathread-60.png)

<div align="center">业务示例</div>

主要代码如下：

![主要代码](assets/javathread-61.png)
...
![直接办理](assets/javathread-62.png)

<div align="center">直接办理</div>

1. 老三发现没有空闲的窗口，就在排队区排队等。

![排队等待](assets/javathread-63.png)
...
![线程池执行流程](assets/javathread-66.png)线程池执行流程
...
![线程池参数](assets/javathread-67.png)
...
![线程池常用阻塞队列](assets/javathread-69.png)
...
![常见线程池参数配置方案-来源美团技术博客](assets/javathread-70.png)
...
![四大线程池](assets/javathread-71.png)
...
![SingleThreadExecutor运行流程](assets/javathread-72.png)
...
![FixedThreadPool](assets/javathread-73.png)
...
![CachedThreadPool执行流程](assets/javathread-74.png)
...
![ScheduledThreadPool执行流程](assets/javathread-75.png)
...
![ScheduledThreadPoolExecutor执行流程](assets/javathread-76.png)
...
![线程池异常处理](assets/javathread-77.png)
...
![线程池状态切换图](assets/javathread-78.png)
...
![JDK 线程池参数设置接口来源参考[7]](assets/javathread-79.png)

<div align="center">JDK 线程池参数设置接口来源参考[7]</div>

这里主要有两个思路：

![动态修改线程池参数](assets/javathread-80.png)
...
![线程池评估方案 来源参考[7]](assets/javathread-81.png)
...
![线程池调优](assets/javathread-82.png)
...
![线程池主要实现流程](assets/javathread-83.png)
...
![自定义线程池](assets/javathread-84.png)
...
![Fork/Join分治算法](assets/javathread-85.png)
