...
![并行和并发](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-1.png)并行和并发

就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。

![并行并发和食堂打饭](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-2.png)并行并发和食堂打饭
...
![程序进程线程关系](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-3.png)

<div align="center">程序进程线程关系</div>
...
![线程创建三种方式](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-4.png)

<div align="center">线程创建三种方式</div>
...
![start方法](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-5.png)

<div align="center">start方法</div>

**为什么我们不能直接调用run()方法？**也很清楚， 如果直接调用Thread的run()方法，那么run方法还是运行在主线程中，相当于顺序执行，就起不到多线程的效果。

### 5.线程有哪些常用的调度方法？

![线程常用调度方法](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-6.png)

<div align="center">线程常用调度方法</div>
...
- `wait()`：当一个线程A调用一个共享变量的 `wait()`方法时， 线程A会被阻塞挂起， 发生下面几种情况才会返回 ：
  ...
  - （2）其他线程调用了线程A的 `interrupt()` 方法，线程A抛出`InterruptedException`异常返回。
...
- `join()`：如果一个线程A执行了`thread.join()`语句，其含义是：当前线程A等待thread线程终止之后才
...
![Java线程状态变化](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-7.png)

<div align="center">Java线程状态变化</div>

### 7.什么是线程上下文切换？

使用多线程的目的是为了充分利用CPU，但是我们知道，并发其实是一个CPU来应付多个线程。

![线程切换-2020-12-16-2107](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-8.png)

<div align="center">线程切换-2020-12-16-2107</div>

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![上下文切换时机](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-9.png)

<div align="center">上下文切换时机</div>
...
![线程间通信方式](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-10.png)

<div align="center">线程间通信方式</div>
...
![ThreadLocal线程副本](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-11.png)

<div align="center">ThreadLocal线程副本</div>
...
![ThreadLoca存放用户上下文](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-12.png)

<div align="center">ThreadLoca存放用户上下文</div>
...
ThreadLocalMap既然被称为Map，那么毫无疑问它是value>型的数据结构。我们都知道map的本质是一个个形式的节点组成的数组，那ThreadLocalMap的节点是什么样的呢？
...
![ThreadLoca结构图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-13.png)

<div align="center">ThreadLoca结构图</div>
...
![ThreadLocal内存分配](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-14.png)

<div align="center">ThreadLocal内存分配</div>
...
![ThreadLocalMap结构示意图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-15.png)

<div align="center">ThreadLocalMap结构示意图</div>
...
![ThreadLocalMap解决冲突](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-16.png)

<div align="center">ThreadLocalMap解决冲突</div>
...
![ThreadLocalMap扩容](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-17.png)

<div align="center">ThreadLocalMap扩容</div>

具体代码：

![ThreadLocalMap resize](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-18.png)

<div align="center">ThreadLocalMap resize</div>
...
![Java内存模型](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-19.png)

<div align="center">Java内存模型</div>

本地内存是JMM的 一个抽象概念，并不真实存在。它其实涵盖了缓存、写缓冲区、寄存器以及其他的硬件和编译器优化。

![实际线程工作模型](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-20.png)

<div align="center">实际线程工作模型</div>
...
![多级指令重排](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-21.png)

<div align="center">多级指令重排</div>
...
![双重校验单例模式异常情形](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-22.png)

<div align="center">双重校验单例模式异常情形</div>
...
![happens-before六大规则](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-23.png)

<div align="center">happens-before六大规则</div>
...
![img](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-24.png)

A和C之间存在数据依赖关系，同时B和C之间也存在数据依赖关系。因此在最终执行的指令序列中，C不能被重排序到A和B的前面（C排到A和B的前面，程序的结果将会被改变）。但A和B之间没有数据依赖关系，编译器和处理器可以重排序A和B之间的执行顺序。

所以最终，程序可能会有两种执行顺序：

![两种执行结果](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-25.png)

<div align="center">两种执行结果</div>
...
![volatile内存可见性](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-26.png)

<div align="center">volatile内存可见性</div>

> volatile怎么保证有序性的呢？

重排序可以分为编译器重排序和处理器重排序，valatile保证有序性，就是通过分别限制这两种类型的重排序。

![volatile重排序规则表](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-27.png)

<div align="center">volatile重排序规则表</div>
...
![volatile写插入内存屏障后生成的指令序列示意图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-28.png)

<div align="center">volatile写插入内存屏障后生成的指令序列示意图</div>

![volatile写插入内存屏障后生成的指令序列示意图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-29.png)

<div align="center">volatile读插入内存屏障后生成的指令序列示意图</div>
...
![monitorenter和monitorexit](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-30.png)

<div align="center">monitorenter和monitorexit</div>

1. synchronized修饰同步方法时，JVM采用`ACC_SYNCHRONIZED`标记符来实现同步，这个标识指明了该方法是一个同步方法。

同样可以写段代码反编译看一下。

![synchronized修饰同步方法](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-31.png)

<div align="center">synchronized修饰同步方法</div>
...
![就诊-图片来源参考[18]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-32.png)

<div align="center">就诊-图片来源参考[18]</div>
...
![Java Montior机制](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-33.png)

<div align="center">Java Montior机制</div>
...
![Mark Word变化](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-34.png)

<div align="center">Mark Word变化</div>
...
![锁升级方向](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-35.png)

<div align="center">锁升级方向</div>
...
![锁升级简略过程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-36.png)锁升级简略过程

完整的升级过程：

![synchronized 锁升级过程-来源参考[14]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-37.png)

<div align="center">synchronized 锁升级过程-来源参考[14]</div>
...
![synchronized和ReentrantLock的区别](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-38.png)

<div align="center">synchronized和ReentrantLock的区别</div>
...
![AQS抽象队列同步器](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-39.png)

<div align="center">AQS抽象队列同步器</div>

![CLH队列](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-40.png)

<div align="center">CLH队列</div>

AQS 中的队列是 CLH 变体的虚拟双向队列，通过将每条请求共享资源的线程封装成一个节点来实现锁的分配：

![AQS变种CLH队列](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-41.png)

<div align="center">AQS变种CLH队列</div>
...
![ReentrantLock 非公平锁加锁流程简图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-42.png)

<div align="center">ReentrantLock 非公平锁加锁流程简图</div>
...
![公平锁tryAcquire](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-43.png)

<div align="center">公平锁tryAcquire</div>
...
![CAS三大问题](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-44.png)

<div align="center">CAS三大问题</div>
...
![Java保证原子性方法](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-45.png)

<div align="center">Java保证原子性方法</div>
...
![原子操作类](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-46.png)

<div align="center">原子操作类</div>
...
![死锁示意图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-47.png)

<div align="center">死锁示意图</div>

那么为什么会产生死锁呢？ 死锁的产生必须具备以下四个条件：

![死锁产生必备四条件](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-48.png)

<div align="center">死锁产生必备四条件</div>
...
![线程死锁检测](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-49.png)

<div align="center">线程死锁检测</div>
...
![王者荣耀等待玩家确认-来源参考[18]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-50.jpeg)

<div align="center">王者荣耀等待玩家确认-来源参考[18]</div>
...
![王者荣耀-来源参考[18]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-51.jpeg)

<div align="center">王者荣耀-来源参考[18]</div>
...
![想结衣结衣不依,迷爱理爱理不理。](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-52.png)

<div align="center">想结衣结衣不依,迷爱理爱理不理。</div>

我们拿代码模拟这一场景，发现CountDownLatch无能为力了，因为CountDownLatch的使用是一次性的，无法重复利用，而这里等待了两次。此时，我们用CyclicBarrier就可以实现，因为它可以重复利用。

![小约翰可汗选妃模拟代码](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-53.png)

<div align="center">小约翰可汗选妃模拟代码</div>

运行结果：

![运行结果](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-54.png)运行结果



<div align="center">CyclicBarrier最最核心的方法，仍然是await()：</div>

- 如果当前线程不是第一个到达屏障的话，它将会进入等待，直到其他线程都到达，除非发生**被中断**、**屏障被拆除**、**屏障被重设**等情况；

上面的例子抽象一下，本质上它的流程就是这样就是这样：

![CyclicBarrier工作流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-55.png)

<div align="center">CyclicBarrier工作流程</div>
...
![停车场空闲车位提示-图片来源网络](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-56.jpeg)

<div align="center">停车场空闲车位提示-图片来源网络</div>

我们把这个例子类比一下，车辆就是线程，进入停车场就是线程在执行，离开停车场就是线程执行完毕，看见红灯就表示线程被阻塞，不能执行，Semaphore的本质就是**协调多个线程对共享资源的获取**。

![Semaphore许可获取-来源参考[18]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-57.jpeg)

<div align="center">Semaphore许可获取-来源参考[18]</div>
...
![英雄交换猎物-来源参考[18]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-58.jpeg)

<div align="center">英雄交换猎物-来源参考[18]</div>
...
![管理线程的池子](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-59.png)

<div align="center">管理线程的池子</div>
...
![业务示例](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-60.png)

<div align="center">业务示例</div>

主要代码如下：

![主要代码](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-61.png)

<div align="center">主要代码</div>
...
![直接办理](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-62.png)

<div align="center">直接办理</div>

1. 老三发现没有空闲的窗口，就在排队区排队等。

![排队等待](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-63.png)

<div align="center">排队等待</div>

1. 老三发现没有空闲的窗口，等待区也满了，蚌埠住了，经理一看，就让休息的小姐姐赶紧回来上班，等待区号靠前的赶紧去新窗口办，老三去排队区排队。小姐姐比较辛苦，假如一段时间发现他们可以不用接着营业，经理就让她们接着休息。

![排队区满](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-64.png)

<div align="center">排队区满</div>

1. 老三一看，六个窗口都满了，等待区也没位置了。老三急了，要闹，经理赶紧出来了，经理该怎么办呢？

![等待区，排队区都满](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-65.png)

<div align="center">等待区，排队区都满</div>
...
![线程池执行流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-66.png)线程池执行流程
...
![线程池参数](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-67.png)

<div align="center">线程池参数</div>
...
![四种策略](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-68.png)

<div align="center">四种策略</div>
...
![线程池常用阻塞队列](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-69.png)

<div align="center">线程池常用阻塞队列</div>
...
![常见线程池参数配置方案-来源美团技术博客](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-70.png)

<div align="center">常见线程池参数配置方案-来源美团技术博客</div>
...
![四大线程池](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-71.png)

<div align="center">四大线程池</div>
...
![SingleThreadExecutor运行流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-72.png)

<div align="center">SingleThreadExecutor运行流程</div>
...
![FixedThreadPool](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-73.png)

<div align="center">FixedThreadPool</div>
...
![CachedThreadPool执行流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-74.png)

<div align="center">CachedThreadPool执行流程</div>
...
![ScheduledThreadPool执行流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-75.png)

<div align="center">ScheduledThreadPool执行流程</div>
...
![ScheduledThreadPoolExecutor执行流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-76.png)

<div align="center">ScheduledThreadPoolExecutor执行流程</div>
...
![线程池异常处理](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-77.png)

<div align="center">线程池异常处理</div>
...
![线程池状态切换图](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-78.png)

<div align="center">线程池状态切换图</div>
...
![JDK 线程池参数设置接口来源参考[7]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-79.png)

<div align="center">JDK 线程池参数设置接口来源参考[7]</div>

这里主要有两个思路：

![动态修改线程池参数](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-80.png)

<div align="center">动态修改线程池参数</div>
...
![线程池评估方案 来源参考[7]](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-81.png)

<div align="center">线程池评估方案 来源参考[7]</div>
...
![线程池调优](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-82.png)

<div align="center">线程池调优</div>
...
![线程池主要实现流程](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-83.png)

<div align="center">线程池主要实现流程</div>
...
![自定义线程池](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-84.png)

<div align="center">自定义线程池</div>
...
![Fork/Join分治算法](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-85.png)

<div align="center">Fork/Join分治算法</div>
...
![工作窃取](https://halo-md.oss-cn-guangzhou.aliyuncs.com/halo/javathread-86.png)

<div align="center">工作窃取</div>
