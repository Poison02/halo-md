...
![img](./MySQL内存篇总结.assets/缓冲池.drawio.png)
...
![img](./MySQL内存篇总结.assets/freelist.drawio.png)
...
![img](./MySQL内存篇总结.assets/Flush.drawio.png)
...
![img](./MySQL内存篇总结.assets/young%2Bold.png)
...
![img](./MySQL内存篇总结.assets/lrutwo.drawio.png)

现在有个编号为 20 的页被预读了，这个页只会被插入到 old 区域头部，而 old 区域末尾的页（10号）会被淘汰掉。

![img](./MySQL内存篇总结.assets/lrutwo2.png)

如果 20 号页一直不会被访问，它也没有占用到 young 区域的位置，而且还会比 young 区域的数据更早被淘汰出去。

如果 20 号页被预读后，立刻被访问了，那么就会将它插入到 young 区域的头部，young 区域末尾的页（7号），会被挤到 old 区域，作为 old 区域的头部，这个过程并不会有页被淘汰。

![img](./MySQL内存篇总结.assets/lrutwo3.png)
...
![img](./MySQL内存篇总结.assets/lruthree.drawio.png)

在批量访问这些数据的时候，会被逐一插入到 young 区域头部。

![img](./MySQL内存篇总结.assets/lruthree1.png)
