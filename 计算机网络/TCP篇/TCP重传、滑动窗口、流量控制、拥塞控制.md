![img](TCP重传、滑动窗口、流量控制、拥塞控制.assets/3.jpg)

------

## [#](https://xiaolincoding.com/network/3_tcp/tcp_feature.html#重传机制)重传机制

TCP 实现可靠传输的方式之一，是通过序列号与确认应答。

在 TCP 中，当发送端的数据到达接收主机时，接收端主机会返回一个确认应答消息，表示已收到消息。

![正常的数据传输](TCP重传、滑动窗口、流量控制、拥塞控制.assets/4.jpg)
...
![超时重传的两种情况](TCP重传、滑动窗口、流量控制、拥塞控制.assets/5.jpg)

> 超时时间应该设置为多少呢？

我们先来了解一下什么是 `RTT`（Round-Trip Time 往返时延），从下图我们就可以知道：

![RTT](TCP重传、滑动窗口、流量控制、拥塞控制.assets/6.jpg)
...
![超时时间较长与较短](TCP重传、滑动窗口、流量控制、拥塞控制.assets/7.jpg)
...
![RTO 应略大于 RTT](TCP重传、滑动窗口、流量控制、拥塞控制.assets/8.jpg)
...
![RFC6289 建议的 RTO 计算 ](TCP重传、滑动窗口、流量控制、拥塞控制.assets/9.jpg)
...
![快速重传机制](TCP重传、滑动窗口、流量控制、拥塞控制.assets/10.jpg)
...
![选择性确认](TCP重传、滑动窗口、流量控制、拥塞控制.assets/11.jpg)
...
![ACK 丢包](TCP重传、滑动窗口、流量控制、拥塞控制.assets/12.jpg)
...
![网络延时](TCP重传、滑动窗口、流量控制、拥塞控制.assets/13.jpg)
...
![按数据包进行确认应答](TCP重传、滑动窗口、流量控制、拥塞控制.assets/14.jpg)
...
![用滑动窗口方式并行处理](TCP重传、滑动窗口、流量控制、拥塞控制.assets/15.jpg)
...
![img](TCP重传、滑动窗口、流量控制、拥塞控制.assets/16.jpg)
...
![可用窗口耗尽](TCP重传、滑动窗口、流量控制、拥塞控制.assets/17.jpg)

在下图，当收到之前发送的数据 `32~36` 字节的 ACK 确认应答后，如果发送窗口的大小没有变化，则**滑动窗口往右边移动 5 个字节，因为有 5 个字节的数据被应答确认**，接下来 `52~56` 字节又变成了可用窗口，那么后续也就可以发送 `52~56` 这 5 个字节的数据了。

![32 ~ 36 字节已确认](TCP重传、滑动窗口、流量控制、拥塞控制.assets/18.jpg)

> 程序是如何表示发送方的四个部分的呢？

TCP 滑动窗口方案使用三个指针来跟踪在四个传输类别中的每一个类别中的字节。其中两个指针是绝对指针（指特定的序列号），一个是相对指针（需要做偏移）。

![SND.WND、SND.UN、SND.NXT](TCP重传、滑动窗口、流量控制、拥塞控制.assets/19.jpg)
...
![接收窗口](TCP重传、滑动窗口、流量控制、拥塞控制.assets/20.jpg)
...
![流量控制](TCP重传、滑动窗口、流量控制、拥塞控制.assets/21.png)
...
![img](TCP重传、滑动窗口、流量控制、拥塞控制.assets/22.jpg)
...
![img](TCP重传、滑动窗口、流量控制、拥塞控制.assets/23.jpg)
...
![窗口关闭潜在的危险](TCP重传、滑动窗口、流量控制、拥塞控制.assets/24.jpg)
...
![窗口探测](TCP重传、滑动窗口、流量控制、拥塞控制.assets/25.jpg)
...
![糊涂窗口综合症](TCP重传、滑动窗口、流量控制、拥塞控制.assets/26.png)
...
![慢启动算法](TCP重传、滑动窗口、流量控制、拥塞控制.assets/27.jpg)
...
![拥塞避免](TCP重传、滑动窗口、流量控制、拥塞控制.assets/28.jpg)
...
![img](TCP重传、滑动窗口、流量控制、拥塞控制.assets/cwnd.png)

拥塞发生算法的变化如下图：

![拥塞发送 —— 超时重传](TCP重传、滑动窗口、流量控制、拥塞控制.assets/29.jpg)
...
![快速重传和快速恢复](TCP重传、滑动窗口、流量控制、拥塞控制.assets/拥塞发生-快速重传.drawio.png)
